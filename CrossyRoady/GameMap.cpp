...
void GameMap::DrawSkill(ConsoleGame::AbstractCanvas* canvas) const
{
    ...
    Vec2 dim = ConsoleGame::Font::GetDim(1);
    Vec2 percentCoord = {.x = skillX + 17, .y = skillY + 2};
    ConsoleGame::Font::DrawString(
        canvas, percentStr, percentCoord, 1, 1, (char)14
    );
    ...
    if (gameFlags.skillInUse) {
        ...
        for (size_t i = 0; i < 15; ++i) {
            for (size_t j = 0; j < 15; ++j) {
                (*canvas)[activeY + i][activeX + j] = (char)1;
            }
        }
        gameSprites.skillCur->Draw(canvas, activeSkillCoord);

        for (size_t j = 0; j < 15; ++j) {
            (*canvas)[activeY][activeX + j] = (char)14;
            (*canvas)[activeY + 15][activeX + j] = (char)14;
        }
        for (size_t i = 0; i < 16; ++i) {
            (*canvas)[activeY + i][activeX] = (char)14;
            (*canvas)[activeY + i][activeX + 15] = (char)14;
        }
...
void GameMap::DrawScore(ConsoleGame::AbstractCanvas* canvas) const
{
    ...
    ConsoleGame::Font::DrawString(
        canvas, scoreString, drawCoord, 1, 1, (char)14
    );
}
...
void GameMap::DrawTime(ConsoleGame::AbstractCanvas* canvas) const
{
    ...
    ConsoleGame::Font::DrawString(
        canvas, SecondsToMMSS(gameEventArgs.timeLeft), {5, 34}, 1, 1, (char)14
    );
}
